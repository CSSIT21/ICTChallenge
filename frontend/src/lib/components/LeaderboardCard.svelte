...
>
	...
	<div
		class="bg-opacity-20 w-full py-[22px] pl-20 pr-14 rounded-2xl shadow flex justify-between items-center relative transition-colors {isHighlighted
			? 'color'
	...
	>
		<div class="text-5xl text-white">
			{#if medal !== ''}
				<img
					src={medal}
					class="absolute top-5 left-3 scale-75"
					alt="medal"
				/>
			{/if}
			{name}
